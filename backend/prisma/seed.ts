import { PrismaClient, ResourceType, CohortState } from '@prisma/client';

const prisma = new PrismaClient();

// Helper: create a record only if none matching the predicate exists
...
async function main() {
  ...
  // ── 1. Cohort ──────────────────────────────────────────────────────────────
  // Note: achievements are bootstrapped automatically via AchievementsService.onApplicationBootstrap()
  const COHORT_NAME = 'LaunchPad Fellowship April 2026';
  const { record: cohort, created: cohortCreated } = await findOrCreate(
    () => prisma.cohort.findFirst({ where: { name: COHORT_NAME } }),
  ...
  console.log(cohortCreated ? '✅ Created cohort' : `⏭️  Cohort already exists, skipping`);

  // ── 2. Sessions ────────────────────────────────────────────────────────────
  // Check how many sessions already exist for this cohort.
  const existingSessionCount = await prisma.session.count({ where: { cohortId: cohort.id } });

  if (existingSessionCount > 0) {
    console.log(`⏭️  Sessions already exist (${existingSessionCount}) for cohort, skipping sessions & resources`);
  } else {
    // ── Month 1: Foundations for Professional Excellence ──
    ...
    const session1 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-04-06'),
      },
    });
    ...
    const session2 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-04-13'),
      },
    });
    ...
    const session3 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-04-20'),
      },
    });
    ...
    const session4 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-04-27'),
      },
    });

    // ── Month 2: Career Clarity & Modern Work Skills ──
    ...
    const session5 = await prisma.session.create({
      data: {
        ...
        scheduledDate: new Date('2026-05-09'),
        unlockDate: new Date('2026-05-04'),
      },
    });
    ...
    const session6 = await prisma.session.create({
      data: {
        ...
        scheduledDate: new Date('2026-05-16'),
    ...
    const session7 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-05-18'),
      },
    });
    ...
    const session8 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-05-25'),
      },
    });

    // ── Month 3: Visibility, Opportunity & Career Growth ──
    ...
    const session10 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-06-08'),
      },
    });
    ...
    const session12 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-06-22'),
      },
    });

    // ── Month 4: Sustainability, Innovation & Future Readiness ──
    ...
    const session13 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-06-29'),
      },
    });
    ...
    const session14 = await prisma.session.create({
      data: {
        ...
        scheduledDate: new Date('2026-07-11'),
        unlockDate: new Date('2026-07-06'),
      },
    });
    ...
    const session15 = await prisma.session.create({
      data: {
        ...
        unlockDate: new Date('2026-07-13'),
      },
    });
    ...
    // ── Resources: Session 1 — Ownership Mindset & Leadership at Work ──
    ...
    // ── Resources: Session 2 — Goal Setting & Time Management ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 3 — Effective Communication Skills ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 4 — Storytelling: Leading Early ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 5 — Career Exploration & Career Planning ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 6 — Remote & Cross-Cultural Collaboration ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 7 — Execution at Work ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 8 — Storytelling: Career Pivots & Transitions ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 9 — Personal Branding & Networking ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 10 — Resume, Cover Letters & Interviews ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 11 — Job Search Strategies ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 12 — Building A Global Career from Africa ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 13 — AI for the Workplace ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 14 — Critical Thinking & Systems Thinking ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 15 — Design Thinking & Problem Solving ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    // ── Resources: Session 16 — Career Sustainability & Long-Term Planning ──
    await prisma.resource.createMany({
      data: [
        {
    ...
    console.log('✅ Total resources: 93');
  }

  console.log('\n🎉 Seed completed successfully!');
  console.log('\n📊 Summary:');
  console.log('   Cohort  : LaunchPad Fellowship April 2026 (Apr 11 – Aug 11, 2026)');
  console.log('   Sessions: 16 (Saturdays, across 4 months)');
  console.log('   Resources: 93 (articles & videos, core + optional)');
  console.log('   Achievements: 50 definitions');
  console.log('\n💡 Note: No users are created by the seed.');
  console.log('   Use /setup to create the first admin account.');
}
